        ...
        for i in range(max_periods, len(btc)):
            ...
            if all(crypto['hold'] != 1 for crypto in crypto_holdings.values()):
                ...
                if crypto and crypto_holdings[crypto]['cooldown'] == 0:
                    if crypto == "BTC":
                        ...
                        # sma = SMA(crypto, forecast, 3, 9, 20)
                        # sma.computeSMA()
                        # sma_short, sma_mid, sma_long = sma.result
                    ...
                    if (sma_mid > sma_long and
                        ...
                        crypto_holdings[crypto]['take_profit'] = max(tp, max_forecast)
            ...
            # exit
                ...
                if crypto_holdings[crypto]['reach_stoploss'] == 0 and crypto_low < crypto_holdings[crypto]['stop_loss']:
                    ...
                    crypto_holdings[crypto]['old_stoploss'] = crypto_holdings[crypto]['stop_loss']
                    crypto_holdings[crypto]['stop_loss'] = Decimal(crypto_holdings[crypto]['stop_loss']) * Decimal('0.9995')
                    crypto_holdings[crypto]['cooldown'] = 60

                if crypto_high > crypto_holdings[crypto]['break_even']:
                    crypto_holdings[crypto]['reach_even'] = 1
                    crypto_holdings[crypto]['stop_loss'] = crypto_holdings[crypto]['break_even']
                    crypto_holdings[crypto]['break_even'] = crypto_holdings[crypto]['break_even'] * Decimal('1.0005')
                ...
                # higher than old stop loss and stop loss is updated == success
                elif crypto_price > crypto_holdings[crypto]['old_stoploss'] and sma_mid < sma_long and crypto_holdings[crypto]['reach_even'] == 1 and crypto_holdings[crypto]['cooldown'] == 0:
                    success_trades += 1
                ...
                # lower than new stop loss: sucess only if higher than old stop loss
                elif crypto_price < crypto_holdings[crypto]['stop_loss'] and sma_mid < sma_long and crypto_holdings[crypto]['reach_even'] == 1 and crypto_holdings[crypto]['cooldown'] == 0:
                    if crypto_price > crypto_holdings[crypto]['old_stoploss']:
                        success_trades += 1
                ...
                # lower than stop loss and not updated == fail
                elif crypto_price < crypto_holdings[crypto]['stop_loss'] and sma_mid < sma_long and crypto_holdings[crypto]['reach_even'] == 0 and crypto_holdings[crypto]['cooldown'] == 0:
                    fail_trades += 1
